...
# additional parameters for ECD
@click.option('--ecd',           help='Activate Easy Consistency Distillation [default: False]',    type=bool, default=False, show_default=True)
@click.option('--mu_schedule',   help='Type of mu scheduling', metavar='stel_lr|sigmoid|const',     type=click.Choice(['step_lr', 'sigmoid', 'const']), default='step_lr', show_default=True)
@click.option('-double_mu',      help='How fast DM to DM', metavar='TICKS',                         type=click.IntRange(min=1), default=100, show_default=True)
@click.option('-initial',        help='Initial value of step_lr', metavar='FLOAT',                  type=click.FloatRange(), default=0.7, show_default=True)
@click.option('-gamma',          help='Gamma of step_lr', metavar='FLOAT',                          type=click.FloatRange(), default=0.5, show_default=True)
@click.option('-ctm',            help='Use CTM parameterization, [default: False]',                 type=bool, default=False, show_default=True)
@click.option('-dpm_solver',     help='dpm solver, [default: False]',                               type=bool, default=False, show_default=True)
@click.option('-heun',           help='Use heun score, [default: False]',                           type=bool, default=False, show_default=True)
@click.option('-norm',           help='Use heun score, [default: False]',                           type=bool, default=False, show_default=True)
@click.option('-conv',           help='Use heun score, [default: False]',                           type=bool, default=False, show_default=True)
@click.option('-qkv',            help='Use heun score, [default: False]',                           type=bool, default=False, show_default=True)
@click.option('-skip',           help='Use heun score, [default: False]',                           type=bool, default=False, show_default=True)
...
def main(**kwargs):
    ...
    if opts.ecd:
        c.loss_kwargs = dnnlib.EasyDict(P_mean=opts.mean, P_std=opts.std, q=opts.q, c=opts.c, k=opts.k, b=opts.b, adj=opts.mapping, gamma=opts.gamma, initial=opts.initial, mu_type=opts.mu_schedule, ctm=opts.ctm)
    else:
    ...
    # partial freeze
    c.network_kwargs.update(freeze_norm_layers=opts.norm, freeze_conv_layers=opts.conv, freeze_qkv_projections=opts.qkv, freeze_skip_layers=opts.skip)

    # Preconditioning & loss function.
    if opts.precond == 'ect':
        c.network_kwargs.class_name = 'training.networks.ECDPrecond'
        if opts.ecd:
            c.loss_kwargs.class_name = 'training.loss.ECDLoss'
        else:
    ...
    if opts.ctm:
        c.network_kwargs.update(training_mode='ctm')
        c.network_kwargs.update(linear_probing=False) 
        c.update(ctm=True) 
    ...
    if opts.ecd:
        c.update(double_ticks_mu=opts.double_mu)
    ...
    dist.print0(f'Setted double mu:       {c.double_ticks_mu}')
    dist.print0()
