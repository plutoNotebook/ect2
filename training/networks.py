...
from .nn import append_dims, dev
...
class EDMPrecond_CTM(torch.nn.Module):
    def __init__(self,
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        self.sigma_max = sigma_max
        self.sigma_data = sigma_data
        self.eye = torch.eye(self.label_dim, device=dev())
        if teacher:
            import pickle
...
class ECDPrecond(torch.nn.Module):
    def __init__(self,
                 img_resolution,                     # Image resolution.
                 img_channels,                       # Number of color channels.
                 label_dim=0,                        # Number of class labels, 0 = unconditional.
                 ...
                 sigma_data=0.5,                     # Expected standard deviation of the training data.
                 model_type='DhariwalUNet',          # Class name of the underlying model.
                 freeze_norm_layers=False,           # Freeze norm layers?
                 freeze_conv_layers=False,           # Freeze conv layers?
                 freeze_qkv_projections=False,       # Freeze QKV projections?
                 freeze_skip_layers=False,           # Freeze skip layers?
                 **model_kwargs):                    # Keyword arguments for the underlying model.
        super().__init__()
        ...
        # Store the freeze options
        self.freeze_norm_layers = freeze_norm_layers
        self.freeze_conv_layers = freeze_conv_layers
        self.freeze_qkv_projections = freeze_qkv_projections
        self.freeze_skip_layers = freeze_skip_layers
    ...
    def _freeze_layers(self):
        """
        Freeze the layers based on the flags provided during initialization.
        """
        if self.freeze_norm_layers:
            self._freeze_layer_type('norm')
        
        if self.freeze_conv_layers:
            self._freeze_layer_type('conv')
        
        if self.freeze_qkv_projections:
            self._freeze_layer_type('qkv')
        
        if self.freeze_skip_layers:
            self._freeze_layer_type('skip')

    def _freeze_layer_type(self, layer_type):
        """
        Helper function to freeze layers based on their type.
        """
    ...
    def forward(self, x, sigma, sigma_s=None, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        if sigma_s is not None:
            sigma_s = sigma_s.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1, self.label_dim], device=x.device) if class_labels is None else class_labels.to(torch.float32).reshape(-1, self.label_dim)
        dtype = torch.float16 if (self.use_fp16 and not force_fp32 and x.device.type == 'cuda') else torch.float32
        ...
        if sigma_s is not None:
            c_noise_s = sigma_s.log() / 4
            F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), c_noise_s.flatten(), class_labels=class_labels, **model_kwargs)
            assert F_x.dtype == dtype
            D_x = c_skip * x + c_out * F_x.to(torch.float32)
            c_skip = sigma_s / sigma
            c_out = (1. - sigma_s / sigma)
            D_x = c_skip * x + c_out * D_x
        else:
            F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), None, class_labels=class_labels, **model_kwargs) 
            assert F_x.dtype == dtype
            D_x = c_skip * x + c_out * F_x.to(torch.float32)
        return D_x
