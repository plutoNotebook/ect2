...
def generator_fn(
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        if net.model.training_mode == 'ctm':
            denoised = net(x, t_cur, t_next, class_labels).to(torch.float64)
            x = denoised
            if not t_next > 0:
                d = (x - denoised) / t_cur
                dt = t_next - t_cur
            ...
            x = net(x, t_cur, None, class_labels).to(torch.float64)
            if t_next > 0:
...
def training_loop(
    ...
    double_ticks_mu     = None,     # Double Mu of ECM Schedule
    ctm                 = False,    # Use CTM?
    rbatch              = 128,
    stf                 = True
):
    ...
    if dist.get_rank() == 0:
        with torch.no_grad():
            ...
            if ctm:
                sigma_s = torch.ones([batch_gpu], device=device)
                misc.print_module_summary(net, [images, sigma, sigma_s, labels], max_nesting=2)
            else:
                misc.print_module_summary(net, [images, sigma, None, labels], max_nesting=2)
    ...
    def update_scheduler_mu(loss_fn):
        loss_fn.update_schedule_mu(stage_mu)
        dist.print0(f'Update scheduler_mu at {cur_tick} ticks, {cur_nimg / 1e3} kimg, ratio {loss_fn.ratio_mu}')
    
    if double_ticks_mu:
        stage_mu = cur_tick // double_ticks_mu
        update_scheduler_mu(loss_fn)
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                loss = loss_fn(net=ddp, images=batch_images, labels=batch_labels, augment_pipe=augment_pipe, stf=stf, ref_images=images)
        ...
        if double_ticks_mu:
            new_stage_mu = (cur_tick-1) // double_ticks_mu
            if new_stage_mu > stage_mu:
                stage_mu = new_stage_mu
                update_scheduler_mu(loss_fn)
